...
#Almost all the code in this file is borrowed from the bitcoin payments for woocommerce plugin - https://wordpress.org/plugins/bitcoin-payments-for-woocommerce/ by BitcoinWay - http://www.bitcoinway.com
...
if ( !defined( __NAMESPACE__ . '\USE_EXT' ) ) {
    if( extension_loaded( 'gmp' ) ) {
        define( __NAMESPACE__ . '\USE_EXT', 'GMP' );
    } elseif ( extension_loaded( 'bcmath' ) ) {
        define( __NAMESPACE__ . '\USE_EXT', 'BCMATH' );
    }
}

spl_autoload_register( 
    function( $full_obj ) {
        $arr = explode( "\\", $full_obj );
        $obj = end( $arr );
        if( strpos( $full_obj, "Abdussamad\WordPress\Plugins" ) !== false && preg_match( '/^[a-z0-9_]+$/i', $obj ) ) {
            @include "$obj.php";
        }
    }
);
